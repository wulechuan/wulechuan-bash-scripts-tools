function detect-remote-signal-file {
	if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ]; then
		console.error    "function detect-remote-signal-file {}: \$1, \$2, \$3 and \$4 are all required."
		return 1
	fi


	local sshTokenAndHost="$1"
	...
	scp    -q    "$sshTokenAndHost:$signalFileRemoteLocationPath/$signalFileName"    "$signalFileLocalTempStoragePath/"
